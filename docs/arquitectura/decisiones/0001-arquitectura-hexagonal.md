...
└── infraestructura/ # NestJS + adaptadores en memoria (Fase 3A); a futuro BD/Prisma, Redis
```
...
`packages/infraestructura` se inició en la Fase 3A con NestJS mínimo y adaptadores en memoria. Implementa los controladores HTTP que exponen `RegistrarNorma`, `PublicarNorma` y `ConsultarContenidoNorma`, y adaptadores en memoria de los puertos (`RepositorioUsuariosEnMemoria`, `RepositorioNormasEnMemoria`, `RepositorioSuscripcionesEnMemoria`, `GeneradorIdsSecuencial`, `PublicadorEventosNormasEnMemoria`). Depende de `aplicacion` (puertos y casos de uso) y `dominio` (tipos); ni `dominio` ni `aplicacion` dependen de infraestructura ni de NestJS. La identidad se simula con el header `x-usuario-id` (placeholder inseguro de Fase 3A, no autenticación real) y la traducción de razones de fallo a códigos HTTP vive solo en infraestructura. Todavía no hay Prisma, PostgreSQL, Redis ni adaptadores externos reales.
