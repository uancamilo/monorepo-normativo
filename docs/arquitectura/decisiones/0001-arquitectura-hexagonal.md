...
└── infraestructura/ # NestJS + adaptadores en memoria y Prisma/PostgreSQL
```

`packages/aplicacion` es un paquete TypeScript puro iniciado en la fase 2. Ya contiene los puertos de repositorio (`RepositorioUsuarios`, `RepositorioNormas`, `RepositorioSuscripciones`), el puerto de eventos `PublicadorEventosNormas`, el puerto `GeneradorIds` y los casos de uso `RegistrarNorma`, `ConsultarContenidoNorma` y `PublicarNorma`, que serán conectados posteriormente por infraestructura. El puerto `GeneradorIds` permite que `RegistrarNorma` genere identificadores sin acoplar la aplicación a `crypto`, UUID ni a la base de datos. El puerto `PublicadorEventosNormas` deja la sincronización futura del índice público (Algolia) detrás de un puerto de aplicación, sin SDK ni adaptador real. Cuando un evento de aplicación derive en efectos externos reintentables dentro de infraestructura persistente, como sincronizar Algolia, debe resolverse con outbox transaccional o un mecanismo equivalente con garantía transaccional.

`packages/infraestructura` se inició en la Fase 3A con NestJS mínimo y adaptadores en memoria. En Fase 3B incorpora Prisma/PostgreSQL como adaptadores reales de persistencia para los puertos existentes, sin cambiar contratos de aplicación ni reglas de dominio. Implementa los controladores HTTP que exponen `RegistrarNorma`, `PublicarNorma` y `ConsultarContenidoNorma`, adaptadores en memoria para pruebas/local y adaptadores Prisma (`RepositorioUsuariosPrisma`, `RepositorioNormasPrisma`, `RepositorioSuscripcionesPrisma`, `GeneradorIdsUuid`). La publicación persistente usa `UnidadDeTrabajoPublicacionNormaPrisma` para guardar la transición editorial y su evento en una sola transacción; el adaptador independiente de eventos de la Fase 3B fue retirado al quedar sustituido por esta garantía transaccional. Infraestructura depende de `aplicacion` (puertos y casos de uso) y `dominio` (tipos); ni `dominio` ni `aplicacion` dependen de infraestructura, NestJS, Prisma ni PostgreSQL. En Fase 3A la identidad se simulaba con el header `x-usuario-id` (placeholder inseguro); desde la Fase 4A infraestructura usa autenticación real mínima con `Authorization: Bearer <token>` (JWT firmado, ver ADR 0005): el token solo identifica al usuario (`sub`) y los permisos siguen resolviéndose con el `Usuario` del dominio dentro de los casos de uso. Dominio y aplicación siguen sin depender de JWT, NestJS ni HTTP. La traducción de razones de fallo a códigos HTTP vive solo en infraestructura.
...
- Las normas con flujo editorial `PUBLICADA` pueden aparecer en búsqueda pública. La consulta de su contenido completo se decide según el tipo de usuario: los usuarios internos (`SUPERADMINISTRADOR`, `ADMINISTRADOR`, `EDITOR`) no requieren suscripción; `SUSCRIPTOR` requiere pertenecer a una cuenta con una suscripción activa y vigente que habilite su correo normalizado. Ver "Acceso a normas" más abajo para el detalle completo de esta frontera.
...
El acceso al contenido completo de una norma se decide con la política `PoliticaAccesoContenidoNorma`, que combina la frontera de `PoliticaAccesoServicio` con la precondición de norma publicada:
...
- `PoliticaAccesoServicio` decide el resto según el tipo de usuario: `SUPERADMINISTRADOR`, `ADMINISTRADOR` y `EDITOR` son usuarios internos y no requieren suscripción. Para `SUSCRIPTOR`, la suscripción debe habilitar su correo (delegado en `Suscripcion.habilitaUsuario(usuario)` y en el comportamiento de comparación normalizada de `Usuario`) y debe estar activa y vigente: estado `ACTIVA`, `fechaInicio` alcanzada y `fechaFin` no alcanzada, validado mediante `Suscripcion.estaActiva(fechaReferencia)` con el rango temporal `[fechaInicio, fechaFin)`.
