...
`packages/infraestructura` se inició en la Fase 3A con NestJS mínimo y adaptadores en memoria. En Fase 3B incorpora Prisma/PostgreSQL como adaptadores reales de persistencia para los puertos existentes, sin cambiar contratos de aplicación ni reglas de dominio. Implementa los controladores HTTP que exponen `RegistrarNorma`, `PublicarNorma` y `ConsultarContenidoNorma`, adaptadores en memoria para pruebas/local y adaptadores Prisma (`RepositorioUsuariosPrisma`, `RepositorioNormasPrisma`, `RepositorioSuscripcionesPrisma`, `GeneradorIdsUuid`, `PublicadorEventosNormasPrisma`). Depende de `aplicacion` (puertos y casos de uso) y `dominio` (tipos); ni `dominio` ni `aplicacion` dependen de infraestructura, NestJS, Prisma ni PostgreSQL. La identidad se simula con el header `x-usuario-id` (placeholder inseguro de Fase 3A, no autenticación real) y la traducción de razones de fallo a códigos HTTP vive solo en infraestructura.
