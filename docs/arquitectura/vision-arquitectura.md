...
- Las normas con flujo editorial `PUBLICADA` pueden aparecer en búsqueda pública. `SUPERADMINISTRADOR`, `ADMINISTRADOR` y `EDITOR` consultan su contenido como usuarios internos sin suscripción; `SUSCRIPTOR` requiere pertenecer a una cuenta por correo habilitado y tener una suscripción activa y vigente.
...
- **Frontera autenticación/autorización (Fase 4D).** `GuardAutenticacion` solo autentica: verifica el Bearer (firma, expiración, `sub`) y entrega la identidad; no valida roles ni decide permisos, y el claim `rol` del token es informativo. La autorización vive en aplicación/dominio. `PoliticaAccesoServicio` centraliza que `SUPERADMINISTRADOR`, `ADMINISTRADOR` y `EDITOR` son usuarios internos sin requisito de suscripción, mientras `SUSCRIPTOR` necesita correo habilitado en una suscripción con `clienteId`, `ACTIVA` y vigente. `PoliticaAccesoContenidoNorma` combina esa regla con una norma `PUBLICADA`; los permisos editoriales siguen resolviéndose con `PoliticaGestionEditorialNorma`. HTTP (`mapeo-http`) solo traduce razones a códigos (identidad inexistente `401`; denegaciones de negocio `403` genérico; inexistente `404`; conflicto `409`), sin reglas propias. RBAC granular, refresh tokens, revocación, logout, sesiones y auth de frontend siguen fuera de alcance.
...
- `EdicionRegistroOficial` es el catálogo interno y la dueña de `urlPdf`. Desde la Fase 5B, `POST /ediciones-registro-oficial/resolver-pendientes` (Bearer, solo SUPERADMINISTRADOR) resuelve esa fuente contra el catálogo oficial mediante el caso de uso puro `ResolverFuenteRegistroOficial`. El puerto `CatalogoRegistroOficial` es de solo lectura y devuelve un resultado discriminado puro de aplicación (`{ exitoso: true; candidatas }` o `{ exitoso: false; razon: 'CATALOGO_TEMPORALMENTE_NO_DISPONIBLE' | 'RESPUESTA_CATALOGO_INVALIDA' | 'COBERTURA_CATALOGO_NO_DISPONIBLE' | 'BUSQUEDA_CATALOGO_INCOMPLETA' }`), de modo que un fallo del catálogo nunca se confunde con "no encontrada". La razón de fallo se conserva íntegra desde el puerto hasta el contrato HTTP agregado: la respuesta de `resolver-pendientes` resume `procesadas/resueltas/noEncontradas/conflictivas/omitidas/erroresCatalogo/erroresPorRazon`, donde `erroresPorRazon` trae siempre las cuatro razones (aunque valgan cero) y `erroresCatalogo` es su suma — no existe un contador genérico `erroresTransitorios`, porque no todos los fallos del catálogo son transitorios; todos, eso sí, dejan la edición `PENDIENTE`. Semántica **fail-closed**: `NO_ENCONTRADA` solo se persiste ante ausencia real (taxonomía cubierta, estructura válida, paginación completa y sin coincidencias descartadas por datos/URLs inválidas); taxonomía no cubierta, HTML inesperado, paginación truncada, URL coincidente inválida o fecha imposible conservan la edición `PENDIENTE`. La consulta lleva tipo + número + fecha detectada (la fecha ubica la carpeta-mes del catálogo y desempata; nunca se reescribe). El adaptador real `CatalogoRegistroOficialHttp` vive solo en infraestructura: consulta el endpoint WordPress `admin-ajax` por carpeta-mes, parsea las *cards* con cheerio, filtra por número y valida las URLs PDF (allowlist de dominio, HTTPS, deduplicación de URLs idénticas). El dominio y la aplicación no conocen HTTP, HTML ni selectores. La persistencia usa compare-and-set (`guardarResolucionSiPendiente`), el lote de pendientes se acota con `listarPendientesSinFuente(limite)` en orden determinista, y el proceso está limitado por máximo de ediciones y paralelismo configurables (con máximos seguros: timeout ≤ 30000 ms, concurrencia ≤ 4, ediciones ≤ 50; fuera de rango ⇒ fail-fast). El adaptador deduplica y cachea localmente cada carpeta-mes (TTL corto, acotada, sin Redis ni persistencia, sin cachear fallos) para no amplificar la paginación por edición; `timeoutMs` es el presupuesto total por carpeta-mes y los reintentos son acotados (máx. 3/página, backoff, `Retry-After`). La integración está deshabilitada por defecto: sin configurarla, el endpoint responde `503 CATALOGO_NO_DISPONIBLE` antes de leer o modificar ediciones; deshabilitada, las demás variables `CATALOGO_REGISTRO_OFICIAL_*` residuales se ignoran (aunque sean inválidas) y la configuración devuelve defaults seguros y deterministas; habilitada pero inválida, el arranque es fail-fast. Seguridad SSRF: la URL base sale solo de configuración, timeout y tamaño de respuesta acotados, redirects no seguidos a ciegas y ninguna URL fabricada ante fallos. Ningún módulo ejecutable inventa URLs; los fakes con candidatos solo existen en tests (ADR 0009).
...
- La Fase 5D cierra ese bloque posterior (`docs/reglas-negocio.md`, sección 17; ADR 0011): `POST /ingesta/registro-oficial/indices/analizar` y `POST /ingesta/registro-oficial/indices/confirmar` (Bearer, solo SUPERADMINISTRADOR) permiten operar el extractor de Fase 5C directamente desde una URL, sin frontend (todavía no existe como repositorio) — solo Postman o `curl`. Dos puertos nuevos y pequeños en aplicación (`DescargadorPdfIndiceRegistroOficial`, `ExtractorIndiceMensualRegistroOficial`) separan la preocupación de red de la de extracción, replicando el mismo principio de "componentes con separación mínima" de ADR 0010; ninguno de los dos casos de uso nuevos (`AnalizarIndiceMensualRegistroOficial`, `ConfirmarIngestaIndiceMensualRegistroOficial`) modifica `IngerirResumenRegistroOficial`, `PoliticaIngestaRegistroOficial` ni el extractor de Fase 5C — `Confirmar` los compone por inyección, tratando la ingesta de Fase 5A como caja negra (misma huella, idempotencia y protección ante carreras, sin duplicar esa lógica). `Analizar` no recibe ningún puerto con capacidad de escritura sobre lotes, entradas, normas o ediciones: la ausencia de esa escritura es una garantía estructural del constructor, no solo de comportamiento; sí depende de `RepositorioUsuarios` (puerto persistente de solo lectura) exclusivamente para autenticar y autorizar al actor. El adaptador de descarga (`DescargadorPdfIndiceRegistroOficialHttp`) es nuevo e independiente de `CatalogoRegistroOficialHttp` (Fase 5B): decodifica bytes binarios en vez de texto y, a diferencia de `esOrigenSeguro`, no tiene ninguna excepción de host local — la URL de un PDF real solo se acepta con hostname exacto `esacc.corteconstitucional.gob.ec`, HTTPS obligatorio, sin redirects.

- **Frontera del frontend (ADR 0012).** Next.js actuará como BFF/reverse proxy bajo el **mismo origen**: el navegador solo habla con Next.js y Next.js llama a NestJS server-to-server, con la URL interna del backend como variable exclusivamente server-side (nunca `NEXT_PUBLIC_*`). El Bearer del backend no queda disponible para JavaScript del navegador; la sesión del frontend viaja en cookie `httpOnly`/`Secure`/`SameSite` y son los Route Handlers o Server Actions quienes adjuntan el Bearer. Consecuencia directa para el backend: **NestJS no habilita CORS** en esta topología (las llamadas directas del navegador a NestJS quedan fuera de la arquitectura aprobada) y `GET /auth/me` es la fuente autoritativa para restaurar el perfil de sesión — el claim `rol` del JWT sigue siendo informativo y no autoriza nada, ni en frontend ni en backend. Las mutaciones a través del BFF deben tratar CSRF explícitamente (`SameSite`, verificación de `Origin`/`Host`): `httpOnly` mitiga exfiltración por XSS, no CSRF. Sin refresh token, revocación ni logout de servidor en esta fase; un 401 del backend invalida la sesión del frontend. Rate limiting de `POST /auth/login` sigue siendo condición obligatoria antes de exposición pública (ADR 0006), preferentemente resuelto en gateway/WAF de Azure. Migrar a orígenes distintos exigiría un ADR nuevo con allowlist CORS explícita.
