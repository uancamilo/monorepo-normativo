...
- Nota técnica (Fase 3A, actualizada en Fases 4A/4B): este flujo se expone por HTTP desde `packages/infraestructura` con NestJS. Desde la Fase 4A la identidad usa `Authorization: Bearer <token>` (JWT firmado) en lugar del header simulado `x-usuario-id`; desde la Fase 4B el token se obtiene con `POST /auth/login` (correo + contraseña verificada contra hash scrypt). El token solo identifica al usuario y los permisos siguen resolviéndose con el `Usuario` del dominio. No cambia ninguna regla de negocio. Detalle en `docs/arquitectura/vision-arquitectura.md`, ADR 0005 y ADR 0006.
