...
- Nota técnica (Fase 3A, actualizada en Fase 4A): este flujo se expone por HTTP desde `packages/infraestructura` con NestJS. Desde la Fase 4A la identidad usa `Authorization: Bearer <token>` (JWT firmado) en lugar del header simulado `x-usuario-id`; el token solo identifica al usuario y los permisos siguen resolviéndose con el `Usuario` del dominio. No cambia ninguna regla de negocio. Detalle en `docs/arquitectura/vision-arquitectura.md` y ADR 0005.
